...
Dado que estos hilos comparten recursos globales y deben coordinar sus etapas de ejecución, la clase `Server` inicializa y distribuye los siguientes mecanismos de sincronización:

## 2.1 Mecanismos de Sincronización y Concurrencia

- **Exclusión Mutua** (`threading.Lock`): Protege la sección crítica de I/O sobre el archivo persistente (`STORAGE_FILE`). Al ser un recurso compartido por todos los `ClientHandle`, el Lock garantiza que la lectura y escritura de lotes de apuestas se realice de forma atómica evitando race conditions.

- **Barrera de Sincronización** (`threading.Barrier`): Coordina la fase de cierre global. Como la determinación de ganadores exige que la totalidad de las apuestas del sorteo hayan sido registradas, la barrera detiene la ejecución de los hilos que terminan su recepción antes mediante `barrier.wait()`. Ningún hilo pasa a la fase de consulta y notificación hasta que el quorum mínimo de agencias (`AGENCY_QUORUM_MIN`) haya completado su envío.

## 2.2 Flujo de Ejecución del Hilo `ClientHandle`
El ciclo de vida de cada hilo de atención se divide en tres fases secuenciales:

1. **Fase de Recepción e Inserción Concurrente:**

    -  El hilo lee lotes de apuestas desde el socket de la agencia en un bucle.

    -  Para cada lote válido, adquiere el mutex (`lock_lottery`), escribe las apuestas en disco (`store_bets`) y libera el cerrojo inmediatamente para no bloquear a otras agencias.

2. **Fase de Sincronización por Quorum:**

    -  Cuando la agencia finaliza la transmisión (retornando `None` o enviando la señal de fin de datos), el hilo sale del bucle de lectura.

    -  Invoca `barrier_agency_quorum.wait()`, quedando suspendido a la espera de que el resto de las agencias terminen de enviar las apuestas.

3. **Fase de Consulta y Notificación de Ganadores:**

    -  Una vez que el último hilo alcanza la barrera, esta se desbloquea simultáneamente.

    -  Cada hilo adquiere el mutex para leer de forma consistente las apuestas consolidadas (`load_bets`), filtra los ganadores correspondientes a su `agency_id`, envía la respuesta (`send_winners`) al cliente y cierra la conexión de forma limpia.
...
- `.clientHandle` / `ServerProtocol`: Módulos propios introducidos para encapsular la lógica de interacción individual por socket y abstraer el parseo de mensajes del protocolo TLV por lotes.
